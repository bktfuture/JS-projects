...
const getCityTemp = async (cityName) => {
	const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=45b6d1548d52a466627f72a82cb325e5&units=${unit}`);
	const data = await res.json();
	renderElements(data);
};

const getCityPic = async (cityName) => {
	const res = await fetch(`https://api.unsplash.com/search/photos/?client_id=76lSVJPvbAXlVHOUl9Pnn4TQmrMWtjqXknZPgpAUEUI&query=${cityName}`);
	const data = await res.json();
...
const renderElements = (data) => {
	...
	tempreature.innerHTML = `${Math.round(data.main.temp)}° <span class="temp-unit">${tempValue}</span>`;
